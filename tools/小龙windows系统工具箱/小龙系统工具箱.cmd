...
:: 获取所有Wi-Fi配置文件名称
netsh wlan show profiles | findstr "所有用户配置文件" > wifi_profiles.txt
...
:: 从文件中提取Wi-Fi名称
set /a index=0
for /f "tokens=3" %%s in (wifi_profiles.txt) do (
    set /a index+=1
    set "wifi_names[!index!]=%%s"
    echo !index! - %%s
)
...
:: 提示用户输入Wi-Fi名称的序号
set /p "user_choice=请输入您想要查看密码的Wi-Fi名称序号: "
if not defined wifi_names[%user_choice%] (
    echo 无效的Wi-Fi名称序号。
    exit /b
)

:: 显示Wi-Fi密码
echo 正在获取 "%wifi_names[%user_choice%]%" 的密码...
netsh wlan show profiles name="%wifi_names[%user_choice%]%" key=clear | findstr "关键内容" > wifi_password.txt

:: 从文件中提取密码
for /f "tokens=3" %%s in (wifi_password.txt) do set wifi_password=%%s
del wifi_password.txt

:: 显示密码
echo "%wifi_names[%user_choice%]%" 的Wi-Fi密码为: %wifi_password%
PAUSE>NUL
goto MENU
...
ECHO.
set TK=
set /p TK=【请输入要结束的 ‘ 进程名 ’或 ‘ PID [ 进程码 ] ’，或者输入 'q' 退出】：
if errorlevel 1 (
    goto open
) else if /i "%TK%"=="q" (
    goto MENU
...
:DELETE_POWER_SCHEME
set /p "PC=请输入要删除电源方案的 GUID，用空格分隔，输入 'q' 退出："
if "%PC%"==" " goto DELETE_POWER_SCHEME
if "%PC%"=="" goto DELETE_POWER_SCHEME
if /i "%PC%"=="q" goto MENU

:: 分割输入的GUID列表
for /f "tokens=* delims= " %%GUID in ("%PC%") do (
