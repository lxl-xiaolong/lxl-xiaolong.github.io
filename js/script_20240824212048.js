...
layui.use(['element', 'layer', 'util'], function() {
    ...
    function generateLinks(links) {
        return links.map(link => `<a href="${link.url}" target="_blank" class="link-item">${link.name}</a>`).join('');
    }
    ...
    function displaySearchResults(results, index) {
        ...
        const html = results.map(result => `
            <div class="search-result-item">
                <a style="padding: 10px; margin-top: 5px;" href="${result.url}" target="_blank" class="search-result-link">${result.name} (${result.category})</a>
            </div>
        `).join('');
        layer.open({
            type: 1,
