...
layui.use(['element', 'layer', 'util'], function() {
    ...
    var loading = layer.msg('正在加载中，请稍后...', { icon: 16, shade: [0.3, '#808080'] });
    ...
    function generateLinks(links) {
        return links.map(link => `<a style="display: block; margin-top: 5px;" href="${link.url}" target="_blank" class="link-item">${link.name}</a>`).join('');
    }
    ...
    function displaySearchResults(results, index) {
        ...
        const html = results.map(result => `
            <div class="search-result-item">
                <a style="padding: 10px; display: block; margin-top: 5px;" href="${result.url}" target="_blank" class="search-result-link">${result.name} (${result.category})</a>
            </div>
        `).join('');
        layer.open({
            type: 1,
